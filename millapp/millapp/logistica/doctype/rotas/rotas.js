...
class ControleDeFormulario {
    ...
    criarRota() {
        console.log(this.frm.doc.data, this.frm.doc.responsavel_de_atendimento)
        frappe.call({
...
class CentralDeDados {
    ...
    marcarHorario(dado) {
        frappe.prompt([
            ...
            // Localizar o dado na lista de atendimentos e atualizar o horário
            console.log(dado)
    ...
    chegadaNoDestinho(dado) {
        ...
        if (linha) {
            ...
            frappe.confirm('Deseja realmente marcar a chegada no destino?', () => {
                this.frm.set_value('check_swapper_edicao', !this.frm.doc.check_swapper_edicao);
                console.log(frappe.datetime.now_time())
